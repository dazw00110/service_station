...
public class AddAutaTabController implements Initializable {

    /** Domyślny konstruktor */
    public AddAutaTabController() {
    }
 ...
 * @param e zdarzenie kliknięcia LPM na przycisk Dodaj */
    public void addAuta(MouseEvent e) {
        try {
            ...
            if (make.isEmpty() || carmodel.isEmpty() || manufacture == null || fault == null || engineType == null || mechanic == null) {
            ...
            String sqlAddAuta = "INSERT INTO public.pojazdy(idmechanika, marka, model, typsilnika, rokprodukcji, opisusterki) VALUES (?, ?, ?, ?, ?, ?);";
            ...
            try (Connection con = DriverManager.getConnection(urlDB, username, password);
                 PreparedStatement pst = con.prepareStatement(sqlAddAuta)) {

                pst.setInt(1, getID(mechanik.getValue()));
                pst.setString(2, make);
                pst.setString(3, carmodel);
                pst.setString(4, typsilnika.getValue());
                pst.setString(5, manufacture);
                pst.setString(6, fault);

                if(checkEngine(mechanik.getValue(), typsilnika.getValue())){
                    pst.execute();
                    Alert alert = new Alert(Alert.AlertType.INFORMATION, "POMYŚLNIE DODANO AUTO!", ButtonType.OK);
                    alert.setHeaderText(null);
                    alert.setTitle("BŁĄD");
                    alert.show();
                } else {
 ...
 * @return ID mechanika*/
    public int getID(String mechanic) {
        String sqlGetMechanicy = "select nazwisko, id from mechanicy";
        String urlDB = "jdbc:postgresql://localhost:5432/warsztat";
        String username = "postgres";
        String password = "student";
...

        {
            try {
                ...
                while (resultGM.next()) {
                    String nazwisko = resultGM.getString("nazwisko");
                    int id = Integer.parseInt(resultGM.getString("id"));

                    if(mechanic.contains(nazwisko)){
                        wynik = id;
                    }
                }
            } catch (SQLException e) {
